...
export function ProductDetailClient({ product, promoBreadcrumb }: { product: Product; promoBreadcrumb?: boolean }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/track", {
      ...
      body: JSON.stringify({ type: "page_view", session_id: sessionId, page: `/product/${product.slug}`, referrer: document.referrer }),
    }).catch(() => {});
  }, [product.slug]);
  ...
  let features: string[] = [];
  try {
    features = t?.features ? (typeof t.features === "string" ? JSON.parse(t.features) : t.features) : [];
  } catch { features = []; }
  ...
  try {
    ...
    if (Array.isArray(raw)) {
      specsEntries = raw.map((item: string) => {
        const colonIdx = item.indexOf(":");
        return colonIdx > 0 ? [item.substring(0, colonIdx).trim(), item.substring(colonIdx + 1).trim()] : [item, ""];
      });
    ...
    <main className="mx-auto max-w-[1440px] px-4 sm:px-6 lg:px-8 py-8 bg-white flex-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[480px_1fr] gap-8">
              ...
              {allImages.map((img, idx) => (
                <button
                  ...
                  className={`relative h-16 w-16 flex-shrink-0 rounded-lg overflow-hidden border-2 transition-all ${
                    selectedImage === img ? "border-purple-700" : "border-gray-200 hover:border-purple-400"
                  }`}
                >
                  <SafeImage src={img} alt="" fill className="object-cover" sizes="64px" loading="lazy" />
                </button>
              ))}
            </div>
          )}
        ...
        <div>
            ...
            {(discount ?? 0) > 0 && discountLabel && (
              <div className="mt-2">
                ...
                {isMultiStoreDiscount ? (
                  <p className="mt-1 text-sm text-gray-500">
                    {language === "zh" ? `相对于${filteredPrices.length}家商城中的最高现价` : `Compared to the highest price from ${filteredPrices.length} stores`}
                  </p>
                ) : (
                  <p className="mt-1 text-sm text-gray-500">
                    {language === "zh" ? "相对于原价" : "Compared to the original price"}
                  </p>
                )}
              </div>
            )}
          ...
          {t?.description && (
            <div className="mt-6">
              <h2 className="text-lg font-semibold text-gray-900 mb-2">{language === "zh" ? "产品描述" : "Description"}</h2>
            ...
            return (
              <div key={price.id} className={`border-t border-gray-100 transition-colors hover:bg-gray-50 ${isLowest ? "bg-emerald-50/50" : ""}`}>
                ...
                <div className="hidden md:grid grid-cols-7 gap-4 px-5 py-4 items-center">
                  ...
                  <div className="text-center">
                    {priceDiscount ? (
                      <span className="inline-block rounded-md bg-red-50 px-2 py-0.5 text-sm font-semibold text-red-600">-{priceDiscount}%</span>
                    ) : (
                      <span className="text-sm text-gray-400">—</span>
                    )}
                  </div>
        ...
        <div className="mt-12">
          ...
          <div className="rounded-2xl border border-gray-200 bg-white overflow-hidden shadow-sm">
            {specsEntries.map(([key, value], idx) => (
              <div key={key} className={`flex items-center px-5 py-3 ${idx > 0 ? "border-t border-gray-100" : ""} ${idx % 2 === 0 ? "bg-gray-50" : ""}`}>
