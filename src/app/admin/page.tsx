...
const CURRENCY_OPTIONS: CurrencyOption[] = [
  { code: 'USD', symbol: '$', flag: '/flags/us.png', flagAlt: 'US' },
  { code: 'JPY', symbol: '¥', flag: '/flags/jp.png', flagAlt: 'JP' },
  { code: 'GBP', symbol: '£', flag: '/flags/gb.png', flagAlt: 'GB' },
  { code: 'EUR', symbol: '€', flag: '/flags/eu.png', flagAlt: 'EU' },
  { code: 'CAD', symbol: 'CA$', flag: '/flags/ca.png', flagAlt: 'CA' },
  { code: 'AUD', symbol: 'A$', flag: '/flags/au.png', flagAlt: 'AU' },
  { code: 'RUB', symbol: '₽', flag: '/flags/ru.png', flagAlt: 'RU' },
  { code: 'KRW', symbol: '₩', flag: '/flags/kr.png', flagAlt: 'KR' },
  { code: 'IDR', symbol: 'Rp', flag: '/flags/id.png', flagAlt: 'ID' },
];
...
export default function AdminPage() {
  ...
  // Fetch promotions data (separate from fetchAllData for pagination)
  const fetchPromotions = useCallback(async () => {
    try {
      const res = await adminFetch(`/api/admin/promotions?page=${promotionPage}&limit=${PROMOTIONS_PER_PAGE}`);
      const json = await res.json();
      if (json.success) {
        setPromotions(json.data?.promotions || []);
        setPromotionTotal(json.data?.pagination?.total || 0);
      }
    } catch (err) {
  ...
  const handleDeletePromotion = async (id: number) => {
    ...
    try {
      const res = await adminFetch(`/api/admin/promotions?id=${id}`, { method: 'DELETE' });
      const json = await res.json();
  ...
  const tabLabels: Record<Tab, { en: string; zh: string }> = {
    ...
    promotions: { en: 'Promotions', zh: '特惠活动' },
        ...
        <nav className="px-3 space-y-1">
          {(['site_settings', 'products', 'promotions', 'categories', 'stores', 'banners', 'best_vapes', 'news', 'analytics', 'database_backup'] as Tab[]).map((tab) => (
            ...
            >
              ...
              {tab === 'promotions' && <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>}
          ...
          {activeTab === 'database_backup' && (
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Export Data for Crawler */}
                <div className="bg-card border border-border rounded-xl p-6">
                  <h3 className="text-lg font-semibold mb-2">{t('Export Data', '导出数据', adminLang)}</h3>
                  <p className="text-sm text-muted-foreground mb-4">
                    {t('Export products, prices and categories as JSON for crawler sync (includes image URL for comparison)', '导出产品、价格和分类数据为 JSON 供爬虫工具同步（包含图片 URL 用于对比）', adminLang)}
                  </p>
                  <button
                    onClick={async () => {
                      try {
                        const res = await fetch('/api/admin/crawler-sync/export');
                        const result = await res.json();
                        if (result.success) {
                          ...
                          a.href = url;
                          a.download = `crawler-export-${new Date().toISOString().slice(0, 10)}.json`;
                          a.click();
                  ...
                  >
                    📤 {t('Export Data', '导出数据', adminLang)}
                  </button>
                </div>

                {/* Import Sync Package */}
                <div className="bg-card border border-border rounded-xl p-6">
                  <h3 className="text-lg font-semibold mb-2">{t('Import Sync Package', '导入同步包', adminLang)}</h3>
                  <p className="text-sm text-muted-foreground mb-4">
                    {t('Upload crawler sync package JSON (creates new products and updates prices)', '上传爬虫同步包 JSON（创建新产品并更新价格）', adminLang)}
                  </p>
                  <label className="w-full rounded-lg border border-orange-800 bg-orange-900/20 px-4 py-2.5 text-sm font-medium text-orange-400 hover:bg-orange-900/40 transition-colors cursor-pointer block text-center">
                    📥 {t('Import Sync Package', '导入同步包', adminLang)}
                    <input
                      type="file"
                      accept=".json"
                      className="hidden"
                      onChange={async (e) => {
                        ...
                        try {
                          ...
                          const res = await fetch('/api/admin/crawler-sync/import', {
                            method: 'POST',
                            body: formData,
                          });
                          const result = await res.json();
            ...
            <div>
              ...
              <div className="mb-8">
                <div className="flex items-center gap-2 mb-3">
                  <h2 className="text-lg font-semibold">{t('Products', '产品', adminLang)}</h2>
            ...
            <div>
              ...
              {loading ? (
                ...
                <div className="rounded-xl border border-border bg-card overflow-hidden">
                  ...
                  {promotions.length === 0 && (
                    <div className="py-12 text-center text-muted-foreground">
                      {t('No promotions yet. Click "Add Promotion" to create one.', '暂无活动。点击"添加活动"开始创建。', adminLang)}
                    </div>
                  )}
                </div>
              )}
              {/* Pagination */}
              {Math.ceil(promotionTotal / PROMOTIONS_PER_PAGE) > 1 && (
                <AdminPagination
                  currentPage={promotionPage}
                  totalPages={Math.ceil(promotionTotal / PROMOTIONS_PER_PAGE)}
                  total={promotionTotal}
                  onPageChange={(p) => { setPromotionPage(p); }}
                  lang={adminLang}
                />
              )}
            </div>
...
const RichTextEditor = forwardRef<RichTextEditorRef, { value: string; onChange: (v: string) => void }>(function RichTextEditor({ value, onChange }, ref) {
  ...
  useEffect(() => {
    ...
    const openImageCrop = (img: HTMLImageElement) => {
      ...
      sizeLabel.textContent = 'Crop Size:';
      sizeLabel.style.cssText = 'font-size:12px;color:#374151;font-weight:500;';
      ...
      widthInput.min = '50';
      widthInput.max = '2000';
      widthInput.style.cssText = 'width:60px;padding:4px 6px;border:1px solid #d1d5db;border-radius:4px;font-size:12px;text-align:center;';
      ...
      heightInput.style.cssText = 'width:60px;padding:4px 6px;border:1px solid #d1d5db;border-radius:4px;font-size:12px;text-align:center;';
      ...
      sizeApplyBtn.addEventListener('click', () => {
        cropBoxW = Math.max(50, Math.min(2000, parseInt(widthInput.value) || 200));
        cropBoxH = Math.max(50, Math.min(2000, parseInt(heightInput.value) || 200));
        updateCropUI();
      });
      ...
      // Resize crop box handles
      let resizeType: string | null = null;
      let resizeStartX = 0, resizeStartY = 0;
      let startCropW = 0, startCropH = 0;
      ...
      const onHandleDown = (e: MouseEvent) => {
        ...
        e.preventDefault();
        e.stopPropagation();
        resizeType = target.dataset.handle;
      ...
      cropHandles.forEach(h => h.addEventListener('mousedown', onHandleDown));
      ...
      // FIXED: Calculate crop area based on zoom and pan, crop box stays fixed
      cropConfirmCallback = async () => {
        ...
        const imgCenterY = cropCenterY + imgPanY;

        // Image top-left in screen coordinates
        const imgLeft = imgCenterX - imgDisplayW / 2;
        const imgTop = imgCenterY - imgDisplayH / 2;

        // Crop box top-left in screen coordinates
        ...
        const cy = Math.max(0, Math.min(natH - 1, Math.round(srcY)));
        const cw = Math.max(1, Math.min(natW - cx, Math.round(srcW)));
        const ch = Math.max(1, Math.min(natH - cy, Math.round(srcH)));
        ...
        // Create canvas at crop box size (output dimensions)
        const cropCanvas = document.createElement('canvas');
        cropCanvas.width = cropBoxW;
        cropCanvas.height = cropBoxH;
        const cropCtx = cropCanvas.getContext('2d')!;

        // Draw the cropped portion from original image to canvas
        // Source: (cx, cy, cw, ch) on original image
        // Dest: (0, 0, cropBoxW, cropBoxH) on canvas
        cropCtx.drawImage(originalImg, cx, cy, cw, ch, 0, 0, cropBoxW, cropBoxH);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative w-full max-w-3xl max-h-[90vh] bg-card rounded-xl border border-border shadow-xl overflow-hidden flex flex-col">
            ...
            <div ref={scrollContainerRef} className="p-6 overflow-y-auto flex-1 space-y-4">
              ...
              {/* Translations & Cover Images */}
              <div className="border-t border-border pt-4">
                <h3 className="text-sm font-semibold mb-2">{t('Theme Cover Images (Multi-language)', '主题封面图 (多语言)', lang)}</h3>
                <p className="text-xs text-muted-foreground mb-3">{t('Upload cover images for each language. Images should match the card size on frontend.', '为每种语言上传封面图。图片尺寸应与前端卡片一致。', lang)}</p>
              ...
              <div className="border-t border-border pt-4">
                ...
                {linkedPromotionProducts.length > 0 ? (
                  ...
                  <div className="text-xs text-muted-foreground py-4 text-center">
                    {t('No products linked to this promotion yet.', '暂无产品关联此活动。', lang)}
                  </div>
                )}
              </div>
            </div>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative w-full max-w-4xl max-h-[90vh] bg-card rounded-xl border border-border shadow-xl overflow-hidden flex flex-col">
            {/* Header - Fixed */}
            <div className="flex items-center justify-between p-4 border-b border-border shrink-0">
              <h2 className="text-lg font-semibold">{promotionProduct ? translate('Edit Promotion Product', '编辑促销产品', lang) : translate('Add Promotion Product', '添加促销产品', lang)}</h2>
              ...
              <div className="space-y-2">
                ...
                {selectedPromotion && (
                  <p className="text-xs text-muted-foreground">
                    {translate('Default Price:', '默认价格：', lang)} {selectedPromotion.currency || '$'}{selectedPromotion.special_price || '—'}
                  </p>
                )}
              </div>
                        ...
                        {hasMultipleCurrencies ? (
                              ...
                              return (
                                <div key={pIdx} className="rounded-md border border-border/50 bg-card p-2">
                                  <div className="flex items-center justify-between mb-1.5">
                                    <div className="text-xs font-medium text-primary text-left">{p.region || translate('Default', '默认', lang)} ({currencyLabel})</div>
                                    <label className="flex items-center gap-1 cursor-pointer">
                                      ...
                                      />
                                      <span className="text-[10px] text-muted-foreground">{translate('No Quote', '无报价', lang)}</span>
                                    </label>
                                  </div>
                                  <div className="grid grid-cols-2 gap-2 mb-1.5">
                                    ...
                                    <div>
                                      <label className="text-[10px] text-muted-foreground text-left block">{translate('Original Price', '原价', lang)} ({currencyLabel})</label>
                                  ...
                                  <div className="grid grid-cols-2 gap-2">
                                    <div>
                                      <label className="text-[10px] text-muted-foreground text-left block">{translate('Discount %', '折扣 %', lang)}</label>
                                    ...
                                    <div>
                                      <label className="text-[10px] text-muted-foreground text-left block">{translate('Product URL', '产品链接', lang)}</label>
                          ...
                          <div>
                            ...
                            <div className="grid grid-cols-2 gap-2 mb-1.5">
                              ...
                              <div>
                                <label className="text-[10px] text-muted-foreground text-left block">{translate('Original Price', '原价', lang)} ({firstP.currency || '$'})</label>
                        ...
                        <div className="border-t border-border/50 pt-2 mt-2">
                          ...
                          {firstP.time_type === 'time_range' && (
                            <div className="grid grid-cols-2 gap-2 mb-2">
                              <div>
                                <label className="text-[10px] text-muted-foreground block mb-0.5 text-left">{translate('Start Time', '开始时间', lang)}</label>
                                <input 
                                  type="datetime-local" 
                              ...
                              <div>
                                <label className="text-[10px] text-muted-foreground block mb-0.5 text-left">{translate('End Time', '结束时间', lang)}</label>
                                <input 
                                  type="datetime-local" 
                          ...
                          {firstP.time_type === 'countdown' && (
                            <div className="mb-2">
                              <label className="text-[10px] text-muted-foreground block mb-0.5 text-left">{translate('Duration', '时长', lang)}</label>
                              <div className="flex items-center gap-2 bg-secondary rounded border border-border px-2 py-1">
                                ...
                                />
                                <span className="text-xs text-muted-foreground">{translate('days', '天', lang)}</span>
                                <input 
                                  type="number" 
                                  min="0"
                                  max="23"
                                ...
                                />
                                <span className="text-xs text-muted-foreground">{translate('hours', '时', lang)}</span>
                                <input 
                                  type="number" 
                                  min="0"
                                  max="59"
                                  ...
                                  className="w-10 rounded bg-background px-1 py-0.5 text-xs text-center" 
                                />
                                <span className="text-xs text-muted-foreground">{translate('min', '分', lang)}</span>
                                <input 
                                  type="number" 
                                  min="0"
                                  max="59"
                                  ...
                                  className="w-10 rounded bg-background px-1 py-0.5 text-xs text-center" 
                                />
                                <span className="text-xs text-muted-foreground">{translate('sec', '秒', lang)}</span>
                              </div>
                            </div>
                          )}
                          {firstP.time_type === 'countdown' && (
                            <div>
                              <label className="text-[10px] text-muted-foreground block mb-0.5 text-left">{translate('After Countdown Ends', '倒计时结束后', lang)}</label>
                              <select 
                                value={firstP.countdown_action} 
                                onChange={(e) => { 
                                  const newP = [...storePrices]; 
            ...
            <div className="flex items-center justify-end gap-3 p-4 border-t border-border shrink-0">
              <button onClick={() => setOpen(false)} className="px-4 py-2 rounded-md bg-secondary text-sm hover:bg-secondary/80">
                {translate('Cancel', '取消', lang)}
              </button>
              <button onClick={handleSave} disabled={saving} className="px-4 py-2 rounded-md bg-purple-600 text-white text-sm hover:bg-purple-700 disabled:opacity-50">
                {saving ? translate('Saving...', '保存中...', lang) : translate('Save', '保存', lang)}
              </button>
            </div>
          </div>
        </div>
    ...
    if (open && product) {
      ...
      const promoPricesList = ((product.promotion_prices as Array<Record<string, unknown>> | undefined) || []).map((p) => {
        ...
        return {
          ...
          original_price: (p.original_price as string) || '',
          product_url: (p.product_url as string) || '',
          discount_percent: (p.discount_percent as number)?.toString() || '',
                  ...
                  return storeGroups.map((group) => {
                    ...
                    return (
                      <div key={group.storeId} className={`mb-3 p-3 rounded-lg border ${firstP.store_type === 'promotion' ? 'border-purple-500/30 bg-purple-500/5' : 'border-border bg-secondary/30'}`}>
                        ...
                        {firstP.store_type === 'promotion' && (
                          <div className="mt-2 p-2 rounded-lg border border-purple-500/20 bg-purple-500/5">
                            ...
                            {firstP.time_type === 'time_range' && (
                              <div className="grid grid-cols-2 gap-2">
                                <div>
                                  <label className="text-[10px] text-muted-foreground block">{t('Start Time', '开始时间', lang)}</label>
                                  <input type="datetime-local" value={firstP.start_time?.slice(0, 16) || ''}
                                    onChange={(e) => { const newP = [...prices]; for (const idx of group.indices) { newP[idx].start_time = e.target.value; } setPrices(newP); }}
                                    className="mt-0.5 w-full rounded-lg border border-border bg-secondary px-2 py-1 text-xs" />
                                </div>
                                <div>
                                  <label className="text-[10px] text-muted-foreground block">{t('End Time', '结束时间', lang)}</label>
                                  <input type="datetime-local" value={firstP.end_time?.slice(0, 16) || ''}
                                    onChange={(e) => { const newP = [...prices]; for (const idx of group.indices) { newP[idx].end_time = e.target.value; } setPrices(newP); }}
                                    className="mt-0.5 w-full rounded-lg border border-border bg-secondary px-2 py-1 text-xs" />
                                </div>
                              </div>
                            )}
                            {firstP.time_type === 'countdown' && (
                              <div>
                                ...
                                <label className="text-[10px] text-muted-foreground block mb-1">{t('When Expired', '到期后', lang)}</label>
                                <select value={firstP.countdown_action}
                                  onChange={(e) => { const newP = [...prices]; for (const idx of group.indices) { newP[idx].countdown_action = e.target.value as 'convert_to_standard' | 'hide'; } setPrices(newP); }}
